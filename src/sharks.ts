...
export function podRequirement(kind: SharkKind, large: boolean, level: number): number {
  if (large) {
    ...
    // of them: a frilled shark at 8 is not something a working pod rams on the way past, it is a
    // pod's worth of work on a level that caps at 15 - but it leaves room to still be running one
    // down while the rest of the water is happening, which asking for ten did not. The
    // cookiecutter at 5 is past the 4 that merely opens Hunting Mode, so arriving in Hunting Mode
    // is no longer the same as being able to clear one out of the way.
